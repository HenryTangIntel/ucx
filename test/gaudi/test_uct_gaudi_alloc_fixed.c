...
int main() {
    ...
    for (unsigned i = 0; i < num_components; i++) {
        status = uct_md_query_list(components[i], &md_resources, &num_md_resources);
        ...
        for (unsigned j = 0; j < num_md_resources; j++) {
            ...
            if (strstr(md_resources[j].md_name, "gaudi")) {
                ...
                status = uct_md_mem_alloc(md, &length, &address, UCS_MEMORY_TYPE_GAUDI, "test", &memh);
                if (status != UCS_OK) {
                    printf("Failed to allocate memory: %s\n", ucs_status_string(status));
                } else {
                    printf("SUCCESS: Allocated memory at %p, length %zu\n", address, length);
