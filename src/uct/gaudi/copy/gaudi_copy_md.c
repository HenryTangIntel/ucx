...
static ucs_config_field_t uct_gaudi_copy_md_config_table[] = {
    ...
    {"ENABLE_RCACHE", "y",
     "Enable registration cache for improved performance. 'try' is not supported, "
     "and is equivalent to 'n'.",
...
static ucs_status_t uct_gaudi_copy_mem_reg(uct_md_h md, void *address, size_t length,
                                          const uct_md_mem_reg_params_t *params,
                                          uct_mem_h *memh_p);
static ucs_status_t uct_gaudi_copy_mem_dereg(uct_md_h md,
                      ...
                      const uct_md_config_t *md_config, uct_md_h *md_p)
{
    ...
    md = ucs_calloc(1, sizeof(*md), "uct_gaudi_copy_md_t");
    if (NULL == md) {
    ...
    /* Eagerly open the device handle */
    ...
    if (md->hlthunk_fd < 0) {
        ucs_error("Failed to open Gaudi device %d", md->device_index);
    ...
    if (hlthunk_get_hw_ip_info(md->hlthunk_fd, &md->hw_info) != 0) {
        ucs_warn("Failed to get hardware info from hlthunk for device %d",
                 md->device_index);
    ...
    /* Detect advanced capabilities now that device is open */
...
#ifdef ENABLE_STATS
    stat_status = UCS_STATS_NODE_ALLOC(&md->stats, &uct_gaudi_copy_md_stats_class,
                                  ucs_stats_get_root(), "-%p", md);
    if (stat_status != UCS_OK) {
