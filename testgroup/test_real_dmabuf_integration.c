 ...
 * between Gaudi and InfiniBand devices for zero-copy RDMA.
 */
...
#include <ucs/memory/memory    printf("\n=== Test Results ===\n");
    if (status == UCS_OK) {
...
static ucs_status_t register_dmabuf_with_mlx(dmabuf_integration_context_t *ctx)
{
    ...
    reg_params.address = mapped_addr;
    ...
    ucs_status_t status = uct_md_mem_reg_v2(ctx->mlx_md, &reg_params, &ctx->mlx_memh);
    
    if (status == UCS_OK) {
        printf("  ✓ Successfully registered DMA-BUF with MLX MD!\n");
        printf("    → Zero-copy RDMA ready between Gaudi ↔ MLX\n");
        
        /* Verify cross-device memory access */
        ...
        if (read_pattern == test_pattern) {
            ...
            printf("  ✓ MLX can now perform RDMA operations on Gaudi memory!\n");
        } else {
        ...
        printf("  ⚠ UCX MLX MD doesn't support DMA-BUF import yet: %s\n", ucs_status_string(status));
        printf("    Note: This is the missing implementation piece\n");
        
        /* For now, demonstrate manual MLX DMA-BUF registration concept */
        printf("  → Conceptual MLX DMA-BUF flow:\n");
        printf("    1. ibv_reg_dmabuf_mr(mlx_pd, dmabuf_fd, 0, length, IBV_ACCESS_*)\n");
        printf("    2. Use returned MR for RDMA operations\n");
        printf("    3. MLX hardware directly accesses Gaudi device memory\n");
        printf("    4. Zero CPU involvement in data transfers\n");
...
int main(int argc, char **argv)
{
    ...
    if (status == UCS_OK) {
        printf("🎉 SUCCESS: Real DMA-BUF cross-device integration working!\n");
        printf("   ✓ Gaudi memory exported as DMA-BUF\n");
        printf("   ✓ DMA-BUF accessible from multiple devices\n");
        printf("   ✓ Zero-copy RDMA infrastructure ready\n");
    } else {
        printf("⚠ PARTIAL: DMA-BUF infrastructure present but needs real hardware\n");
        printf("   • Requires Gaudi device with DMA-BUF support\n");
        printf("   • Requires InfiniBand driver with ibv_reg_dmabuf_mr support\n");
        printf("   • Requires kernel DMA-BUF framework\n");
    }
    
    printf("\nKey Missing Pieces for Full Integration:\n");
    printf("• Real ibv_reg_dmabuf_mr() implementation in IB driver\n");
    printf("• UCX IB MD integration with DMA-BUF import\n");
    printf("• Cross-device memory registration bridging\n");
    printf("• GPUDirect RDMA peer-to-peer support\n");
