...
static ucs_status_t setup_ucm_events(void)
{
    ...
    // Register mmap/munmap events (skip if not supported)
...
static void test_system_memory_with_ucm(void)
{
    ...
    printf("Note: mmap/munmap test skipped due to portability issues\n");
    
    print_event_summary();
}

#ifdef HAVE_HLTHUNK_H
static void test_gaudi_memory_with_ucm(void)
{
    printf("\n=== Testing Gaudi Memory with UCM ===\n");
    reset_events();
    ...
    // Test Gaudi device memory allocation
    printf("Testing Gaudi device memory allocation...\n");
    uint64_t size = 4096;
    ...
    if (handle != 0) {
        ...
        // Test freeing Gaudi device memory
        printf("Testing Gaudi device memory free...\n");
...
static void test_ucm_query_info(void)
{
    ...
    printf("  Note: Some UCM config functions may not be available in public API\n");
}
...
int main(int argc, char **argv)
{
    ...
    printf("UCX Memory Manager (UCM) Gaudi Integration Test\n");
    printf("===============================================\n");
    ...
    test_gaudi_memory_with_ucm();
    
    // Cleanup
    cleanup_ucm_events();
    
    printf("\n=== UCM Gaudi Integration Test Complete ===\n");
    printf("Check the event summaries above to verify UCM is intercepting\n");
    printf("memory operations correctly for both system and Gaudi memory.\n");
